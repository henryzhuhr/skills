...
如果任务超出这里列出的核心工作流，或需要更完整的 `gh` 命令参考、输出格式建议和通用最佳实践，读取 [references/gh.md](references/gh.md)。

如果问题涉及登录、切换账号、token、scope、权限不足、Git 凭证集成等认证问题，优先读取 [references/gh_auth.md](references/gh_auth.md)。
